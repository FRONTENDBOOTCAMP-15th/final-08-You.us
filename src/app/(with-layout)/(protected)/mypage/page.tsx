...
import { getMyproduct, getMyReviews } from '@/lib/api/mypage';
...
export default function MyPage() {
  ...
  useEffect(() => {
    // 찜목록 불러오기
    const fetchProductsAndReview = async () => {
      const data = await getMyproduct();
      const fetchReview = await getMyReviews();

      const dataItem = data?.item;
      const dataReview = fetchReview?.item;

      setProducts(dataItem);
      setReview(dataReview);
    };

    fetchProductsAndReview();
  }, []);
